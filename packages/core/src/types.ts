...
export type OrderStatus =
  | 'pending_payment'   // Waiting for user to pay (PIX/SPEI/etc.)
  | 'payment_received'  // Anchor received fiat payment
  | 'processing'        // Anchor is processing the conversion
  | 'stellar_pending'   // Transaction submitted to Stellar
...
export type RampEvent =
  | { type: 'quote_received'; quotes: RampQuote[] }
  | { type: 'order_created'; order: RampOrder }
  | { type: 'status_updated'; order: RampOrder }
  | { type: 'order_completed'; order: RampOrder }
  | { type: 'order_failed'; order: RampOrder; error: string }
  | { type: 'error'; error: string; anchorId?: AnchorId };
